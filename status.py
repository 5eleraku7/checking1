    ...
    if response.status_code == 200:
        ...
        if data["success"]:
            ...
            for device in devices:
                username = device.get("user_id")
                kd_country = device.get("country_code")
                ip_address = device.get("ip_address")
                ip_score = device.get("ip_score", 0)
                total_points = device.get("total_points", 0)
                
                print(f"{current_time} - ID: {username}, IP Address: {kd_country} {ip_address}, IP Score: {ip_score}, Total Points: {total_points}")
                line_count += 1
        else:
